...
import { Check, CreditCard, LockKeyhole, Smartphone } from "lucide-react";
...
export default async function BillingPage({ searchParams }: { searchParams: { plan?: string } }) {
  ...
  const subscription = await prisma.billingSubscription.findFirst({ where: { userId: user.id, status: { in: ["active", "attention", "non-renewing"] } }, orderBy: { updatedAt: "desc" } });
  ...
  return (
    <main id="main" className="app-shell min-h-screen px-4 py-10 sm:py-12">
      ...
      <section className="panel mt-4 flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-brand-400/10 text-brand-300"><Smartphone size={18} aria-hidden/></span><div><h2 className="text-sm font-semibold">Paying from Kenya?</h2><p className="mt-1 text-xs app-muted">Get a renewable 30-day Pro pass for {formatPlanPrice(getCheckoutProduct("pro_pass_30d_kes").amount,"KES")} via M-PESA.</p></div></div><div className="w-full sm:w-52"><BillingCheckoutButton productKey="pro_pass_30d_kes" ready={checkoutProductReady("pro_pass_30d_kes")} /></div></section>
      </div>
    </main>
  );
}
