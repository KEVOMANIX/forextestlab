...
- Each chart loads older context in bounded pages as the user scrolls left, up to the instrument's earliest provider data on every supported timeframe. Context candles never affect execution or analytics.
...
to Lightsail. `install-calendar-task.ps1` schedules that bridge once daily at
11:00 local time by default (override with `-DailyAt HH:mm`). On Lightsail, the
matching timer checks for an uploaded file every 30
