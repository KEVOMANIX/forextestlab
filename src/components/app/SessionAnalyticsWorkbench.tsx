...
function PeriodBars({ trades }: { trades: ClosedTrade[] }) {
  if (!trades.length) return <EmptyChart />;
  const periods = new Map<string,number>(); trades.forEach(t=>{const key=new Date(t.exitTime).toISOString().slice(0,7); periods.set(key,(periods.get(key)??0)+Number(t.pnl));});
...
export function SessionAnalyticsWorkbench({ trades, equityCurve, startingBalance }: { trades: ClosedTrade[]; equityCurve: EquityPoint[]; startingBalance: string }) {
  ...
  return <div className="mt-7">
    ...
    {tab==='timing'&&<div className="mt-4 grid gap-4 lg:grid-cols-2"><ChartFrame title="Entry-time heatmap" subtitle="Day of week × hour in UTC" className="lg:col-span-2"><TimingHeatmap trades={filtered}/></ChartFrame><ChartFrame title="Monthly performance" subtitle="Net realised P/L by exit month"><PeriodBars trades={filtered}/></ChartFrame><ChartFrame title="Trade duration & R multiple" subtitle="Find whether longer holds improve outcomes"><RiskScatter trades={filtered}/></ChartFrame></div>}
