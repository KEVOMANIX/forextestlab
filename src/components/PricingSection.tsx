...
import { ArrowRight, CreditCard, ShieldCheck, Smartphone } from "lucide-react";
...
export function PricingSection() {
  return (
    <Section
      ...
      description="Simple Kenya-shilling pricing with secure checkout. No trading commissions, deposits, or hidden platform fees."
    ...
    >
      ...
      <div className="mt-8 grid gap-3 rounded-2xl border border-white/10 bg-surface-900/70 p-4 text-sm text-slate-400 sm:grid-cols-3 sm:p-5">
        <span className="flex items-center gap-2"><Smartphone size={16} className="text-brand-300" aria-hidden /> Pay in Kenya shillings</span>
        <span className="flex items-center gap-2"><CreditCard size={16} className="text-brand-300" aria-hidden /> Secure checkout via Paystack</span>
