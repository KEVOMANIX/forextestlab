  ...
  return (
    <section className="panel mt-4 overflow-hidden">
      ...
      <div className="p-3 sm:p-4">
        ...
        {tab === "activity" && (
          activity.length ? <ol className="grid gap-2 lg:grid-cols-2">{activity.map((item) => <li key={item.id} className="flex items-center gap-3 rounded-xl border app-border bg-[var(--app-panel-2)]/45 p-3"><span className={`grid h-8 w-8 shrink-0 place-items-center rounded-full ${item.positive ? "bg-profit/10 text-profit" : "bg-loss/10 text-loss"}`}>{item.positive ? <TrendingUp size={14} /> : <TrendingDown size={14} />}</span><span className="min-w-0 flex-1"><span className="block truncate text-xs font-semibold capitalize">{item.label}</span><span className="mt-1 block text-[11px] app-muted">{item.date}</span></span><span className={`font-mono text-xs font-semibold ${item.positive ? "text-profit" : "text-loss"}`}>{item.pnl}</span></li>)}</ol> : <div className="rounded-xl bg-[var(--app-panel-2)]/55 px-4 py-8 text-center"><Activity size={20} className="mx-auto text-brand-300" /><p className="mt-3 text-sm font-semibold">No closed trades yet</p><p className="mt-1 text-xs app-muted">Your latest decisions will appear here.</p></div>
        )}

        {tab === "analyst" && (aiEnabled ? <AiInsightsPanel scope="portfolio" suggestions={PORTFOLIO_SUGGESTED_QUESTIONS} title="Ask your trading data" subtitle="AI analysis across all your saved sessions" /> : <div className="relative overflow-hidden rounded-xl border border-brand-400/25 bg-[linear-gradient(120deg,rgba(20,184,166,0.10),rgba(59,107,255,0.08))] p-5"><Bot size={22} className="text-brand-300" /><p className="mt-3 font-semibold">Meet your AI trading analyst</p><p className="mt-1 max-w-2xl text-xs leading-5 app-muted">Pro reviews every saved session, identifies recurring strengths and mistakes, and suggests what to test next.</p><Link href="/account/billing" className="btn-primary mt-4 px-4 py-2 text-xs">Unlock AI analyst</Link></div>)}
      </div>
    </section>
  );
}
