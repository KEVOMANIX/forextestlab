...
function Screenshot({ src, alt, width, height, priority = false }: { src: string; alt: string; width: number; height: number; priority?: boolean }) {
  return <div className="overflow-hidden rounded-2xl border border-white/10 bg-surface-900 p-1.5 shadow-[0_32px_80px_-38px_rgba(0,0,0,.95)]"><Image src={src} alt={alt} width={width} height={height} priority={priority} className="h-auto w-full rounded-lg" sizes="(max-width:1024px) 100vw, 64vw" /></div>;
}
  ...
  return (
    <PageShell>
      ...
      <div className="border-t border-white/10">
        <section id="setup" className="scroll-mt-24 py-12 sm:py-14">
          <div className="container-page grid items-center gap-8 lg:grid-cols-[.72fr_1.28fr] lg:gap-12"><div><span className="font-mono text-sm font-semibold text-brand-300">01</span><p className="mt-4 text-xs font-semibold uppercase tracking-[0.16em] text-brand-300">Set up the test</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-white lg:text-4xl">Give the session a clear scope.</h2><p className="mt-4 text-base leading-7 text-slate-400">Select the market, historical starting point, replay length, simulated balance, and whether the session follows prop-firm rules.</p><Points items={["Choose one or several available markets", "Use a preset period or custom date range", "Name and tag the strategy before replay begins"]} /></div><Screenshot src="/product/session-dashboard-20260814.webp" alt="ForexTestLab session setup and dashboard" width={1600} height={940} /></div>
        </section>

        <section id="replay" className="scroll-mt-24 border-y border-white/10 bg-surface-900/30 py-12 sm:py-14">
          <div className="container-page grid items-center gap-8 lg:grid-cols-[1.28fr_.72fr] lg:gap-12"><Screenshot src="/product/market-replay-20260814-v2.webp" alt="ForexTestLab historical replay" width={1786} height={880} /><div><CandlestickChart size={22} className="text-brand-300" aria-hidden /><span className="mt-4 block font-mono text-sm font-semibold text-brand-300">02</span><p className="mt-3 text-xs font-semibold uppercase tracking-[0.16em] text-brand-300">Replay and execute</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-white lg:text-4xl">Make decisions before the outcome is known.</h2><p className="mt-4 text-base leading-7 text-slate-400">Advance the replay at your pace, inspect synchronized chart context, and place simulated orders using the plan you defined.</p><Points items={["Pause or advance candle by candle", "Use drawing and position-planning tools", "Manage entries, stops, targets, and exits"]} /></div></div>
        </section>

        <section id="journal" className="scroll-mt-24 py-12 sm:py-14">
          <div className="container-page grid items-center gap-8 lg:grid-cols-[.72fr_1.28fr] lg:gap-12"><div><NotebookPen size={22} className="text-brand-300" aria-hidden /><span className="mt-4 block font-mono text-sm font-semibold text-brand-300">03</span><p className="mt-3 text-xs font-semibold uppercase tracking-[0.16em] text-brand-300">Document the decision</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-white lg:text-4xl">Capture what the numbers cannot explain.</h2><p className="mt-4 text-base leading-7 text-slate-400">Journal the setup while the reasoning is fresh, then add the lesson after the position closes.</p><Points items={["Record setup quality, confidence, and emotion", "Attach before-entry and after-exit views", "Separate experimental trades from strategy analytics"]} /></div><Screenshot src="/product/session-analytics.webp" alt="ForexTestLab trade journal and review" width={1600} height={940} /></div>
        </section>

        <section id="review" className="scroll-mt-24 border-y border-white/10 bg-surface-900/30 py-12 sm:py-14">
          <div className="container-page grid items-center gap-8 lg:grid-cols-[1.28fr_.72fr] lg:gap-12"><Screenshot src="/product/session-analytics-20260814.webp" alt="ForexTestLab analytics report" width={1600} height={940} /><div><BarChart3 size={22} className="text-brand-300" aria-hidden /><span className="mt-4 block font-mono text-sm font-semibold text-brand-300">04</span><p className="mt-3 text-xs font-semibold uppercase tracking-[0.16em] text-brand-300">Evaluate the evidence</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-white lg:text-4xl">Turn the session into a specific next step.</h2><p className="mt-4 text-base leading-7 text-slate-400">Review performance patterns, reopen individual trades on the chart, and compare the result with the process recorded in the journal.</p><Points items={["Measure return, expectancy, and drawdown", "Inspect timing, session, direction, and streak patterns", "Translate recurring mistakes into the next test rule"]} /></div></div>
        </section>

        <section id="continue" className="scroll-mt-24 py-12 sm:py-14">
          <div className="container-page grid items-center gap-8 lg:grid-cols-[.72fr_1.28fr] lg:gap-12"><div><Play size={22} className="text-brand-300" aria-hidden /><p className="mt-4 text-xs font-semibold uppercase tracking-[0.16em] text-brand-300">Continue later</p><h2 className="mt-3 text-3xl font-bold leading-tight tracking-tight text-white lg:text-4xl">Resume from the state you saved.</h2><p className="mt-4 text-base leading-7 text-slate-400">The dashboard remembers the selected session and restores its replay progress, trades, drawings, layout, and loaded market history.</p><Link href="/features#sessions" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-brand-300 hover:text-brand-200">View saved-session features <ArrowRight size={15} aria-hidden /></Link></div><Screenshot src="/product/session-dashboard-20260814.webp" alt="ForexTestLab saved sessions" width={1600} height={940} /></div>
        </section>
      </div>
