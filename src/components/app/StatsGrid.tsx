...
export function StatsGrid({ stats }: { stats: PerformanceStats }) {
  const cards: { label: string; value: string; tone?: string }[] = [
    { label: "Net profit / loss", value: money(stats.netProfit), tone: Number(stats.netProfit) >= 0 ? "text-profit" : "text-loss" },
    ...
    { label: "Gross profit", value: money(stats.grossProfit), tone: "text-profit" },
    { label: "Gross loss", value: money(stats.grossLoss), tone: "text-loss" },
    { label: "Average win", value: money(stats.averageWin), tone: "text-profit" },
    { label: "Average loss", value: money(stats.averageLoss), tone: "text-loss" },
    { label: "Largest win", value: money(stats.largestWin), tone: "text-profit" },
    { label: "Largest loss", value: money(stats.largestLoss), tone: "text-loss" },
    { label: "Max drawdown", value: money(stats.maxDrawdown), tone: "text-loss" },
